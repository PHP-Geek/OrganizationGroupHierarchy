    ...
    columns: [
        ...
        {data: null, searchable: false, orderable: false, title: "Action", visible: true,
            render: function (data, full, row) {
                ...
                return html = '<a href="' + base_url + '/admin/product/edit/' + row.id + '" type="button" class="btn blue btn-outline btn-sm"><i class="fa fa-pencil"></i></a><span id="status' + row.id + '" class="fa fa-trash btn red btn-outline btn-sm" onclick="productStatus(' + row.id + ')" data-value="-1"> </span>';
            }
        },
    ]
});
...
function productStatus(Id) {
    swal(
        ...
        if (isConfirm) {
            ...
            $.post(base_url + "/admin/change-product-status", {_token: $("input[name=_token]").val(), id: Id, productStatus: status}, function (data) {
